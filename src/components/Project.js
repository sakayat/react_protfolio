...
const Project = ({ project }) => {
  ...
  return (
    <div className="project grid grid-cols-5">
      <div
        className="project-left flex flex-col col-span-3 gap-10"
        ref={projectLeftRef}
      >
        <span className="text-9xl text-white/20">
          {String(project.id).padStart(2, 0)}
        </span>
        <h3 className="uppercase leading-relaxed text-5xl text-cyan-400">
          {project.title}
        </h3>
        ...
        <div className="link flex flex-wrap gap-10">
          {project.liveLink && (
            <Button project={project} button={"Live Site"} />
          )}
          {project.frontEndLink && (
            <Button project={project} button={"Front-End Site"} />
          )}
          {project.backEndLink && (
            <Button project={project} button={"Back-End Site"} />
          )}
        </div>
      </div>
      <div
